...
class Xtts(BaseTTS):
    ...
    def __init__(self, config: Coqpit):
        ...
        # FIX: Only load mel stats once, store as a buffer
        self.register_buffer("mel_stats", torch.ones(80))
        default_path = "../experiments/clips_mel_norms.pth"
        if Path(default_path).exists():
            loaded_stats = torch.load(default_path, map_location="cpu")
            if isinstance(loaded_stats, torch.Tensor) and loaded_stats.shape == (80,):
                self.mel_stats.copy_(loaded_stats)
        ...
        if self.args.gpt_use_perceiver_resampler:
            ...
            for i in range(0, audio.shape[1], 22050 * chunk_length):
                ...
                if audio_chunk.size(-1) < 22050 * 0.33:
                    continue
                # FIX: pass self.mel_stats explicitly
    ...
    def get_conditioning_latents(
        ...
        for file_path in audio_paths:
            ...
            if librosa_trim_db is not None:
                ...
                trimmed, _ = librosa.effects.trim(audio_np, top_db=librosa_trim_db)
                audio = torch.from_numpy(trimmed).unsqueeze(0).to(self.device)
    ...
    # FIX: Wrap in torch.autocast and optional torch.compile for MPS
    @torch.inference_mode()
    def inference(
        self,
        text,
        ...
        **hf_generate_kwargs,
        ...
        # Optional compile on MPS
        # if device.type == "mps":
        #     if not hasattr(self.gpt, "_compiled"):
        #         self.gpt = torch.compile(self.gpt, mode="reduce-overhead")
        #         setattr(self.gpt, "_compiled", True)
        #     if not hasattr(self.hifigan_decoder, "_compiled"):
        #         self.hifigan_decoder = torch.compile(self.hifigan_decoder, mode="reduce-overhead")
        #         setattr(self.hifigan_decoder, "_compiled", True)

        if isinstance(text, str):
            text = [text]
            language = [language]
        if len(text) != len(language):
            raise ValueError(f"Number of texts ({len(text)}) must match number of languages ({len(language)})")
        ...
        # Run with half precision on MPS if available
        with torch.autocast(device_type="mps", dtype=torch.float16, enabled=(device.type == "mps")):
            text_tokens_list = []
            max_text_length = 0
            ...
            for txt, lang in zip(text, language):
                lang = lang.split("-")[0]
                txt = txt.strip().lower()
                tokens = self.tokenizer.encode(txt, lang=lang)
                text_tokens_list.append(tokens)
                max_text_length = max(max_text_length, len(tokens))
                if len(tokens) >= self.args.gpt_max_text_tokens:
                    raise ValueError(f" ❗ Input text too long for XTTS (max {self.args.gpt_max_text_tokens} tokens)")

            text_tokens = torch.zeros((batch_size, max_text_length), dtype=torch.long, device=device)
            for i, tokens in enumerate(text_tokens_list):
                text_tokens[i, : len(tokens)] = torch.tensor(tokens, dtype=torch.long, device=device)
            ...
            expected_output_len = torch.tensor(
                [gpt_codes.shape[-1] * self.gpt.code_stride_len], device=device
            )
            ...
            wavs = self.hifigan_decoder(gpt_latents, g=speaker_embedding)
        return {
            "wav": wavs.cpu().squeeze().numpy(),
